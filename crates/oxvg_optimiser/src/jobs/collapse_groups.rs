...
use oxvg_ast::{
    ...
    element::{category::ElementCategory, data::ElementId, Element},
...
fn move_attributes_to_child<'input, 'arena>(element: &Element<'input, 'arena>) {
    ...
    for mut attr in attrs.into_iter_mut() {
        ...
        if let Attr::Transform(value) = &mut *attr {
            let Attr::Transform(child_value) = &mut *child_attr else {
                continue;
            };
        ...
        } else if ContentType::Inheritable(Inheritable::Inherited) == child_attr.value() {
            log::debug!("collapse_groups: moved {attr:?}: is explicit inherit");
            *child_attr = attr.clone();
        } else {
            let value = attr.value();
            if !matches!(value, ContentType::Inheritable(_)) && child_attr.value() != value {
                log::debug!(
                    "collapse_groups: removing {attr:?}: inheritable attr is not inherited"
                );
                removals.pop();
                break;
            }
...
fn flatten_when_all_attributes_moved<'input, 'arena>(element: &Element<'input, 'arena>) {
...
fn has_animated_attr<'input, 'arena>(
    element: &Element<'input, 'arena>,
...
fn is_position_visually_unstable<'input, 'arena>(
    ...
    let is_child_transformed_group =
        *child.qual_name() == ElementId::G && has_attribute!(child, Transform);
    is_node_clipping || is_child_transformed_group
}

fn is_node_with_filter<'input, 'arena>(node: &Element<'input, 'arena>) -> bool {
    node.has_attribute(&AttrId::Filter)
