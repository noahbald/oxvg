...
use oxvg_ast::{
    atom::Atom,
    attribute::data::{core::Url, path, Attr, AttrId},
        ...
        for (_, list) in paths.iter_mut() {
            ...
            for attr in list[0].attributes().into_iter() {
                match attr.unaliased() {
                    Attr::Id(id) => {
                        if !hrefs.contains(id) && HasId::has_id(&mut context.stylesheet, id)? {
            ...
            let new_id: Atom<'input> = format!("#{new_id}").into();
            for path in list {
                ...
                path.set_local_name(ElementId::Use, &context.info.allocator);
            }
        }
...
struct Key<'input> {
    path: Path,
    stroke: Option<SVGPaint<'input>>,
    fill: Option<SVGPaint<'input>>,
}
