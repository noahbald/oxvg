...
use oxvg_ast::{
    attribute::data::{inheritable::Inheritable, presentation::LengthPercentage, Attr, AttrId},
    element::Element,
...
impl<'input, 'arena> Visitor<'input, 'arena> for ApplyTransforms {
    ...
    fn use_style(&self, element: &Element<'input, 'arena>) -> bool {
        get_attribute!(element, D).is_some()
    }
    ...
    fn element(
        ...
        for mut attr in element.attributes().into_iter_mut() {
            if matches!(attr.name().unaliased(), AttrId::Id | AttrId::Style) {
            ...
            get_computed_style_css!(computed_styles, Transform(None))
        {
            if (&css_transform)
                .try_into()
                .ok()
                .is_none_or(|css_transform| *transform == css_transform)
            {
        ...
        if let Some((stroke, Mode::Static)) = stroke {
...
impl ApplyTransforms {
    ...
    fn apply_stroked<'input, 'arena>(
        ...
        // FIXME: https://github.com/servo/html5ever/pull/679
        todo!();
        // let vector_effect = get_attribute!(element, VectorEffect);
        // if vector_effect == VectorEffect::NonScalingStroke {
        //     return false;
        // }
        // drop(vector_effect);
        ...
        let mut stroke_width = match stroke_width {
            Some(Inheritable::Defined(value)) => value,
            // NOTE: Default stroke-width value is 1
            None | Some(Inheritable::Inherited) => LengthPercentage::px(1.0),
        };
        stroke_width = LengthPercentage(stroke_width.mul(scale as f32));
        ...
        {
            *stroke_dashoffset = stroke_dashoffset.mul(scale)
        }
