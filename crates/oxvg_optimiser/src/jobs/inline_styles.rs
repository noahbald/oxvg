...
use oxvg_ast::{
    atom::Atom,
    attribute::data::{core::Style, AttrId},
    element::{data::ElementId, Element, HashableElement},
...
impl<'input, 'arena> Visitor<'input, 'arena> for State<'_, 'input, 'arena> {
    ...
    fn exit_element(
        ...
        // TODO: Wait for lightningcss to make this method public
    ...
    fn exit_document(
        ...
        for (element, mut group) in grouping {
            ...
            if !element.has_attribute(&AttrId::Style) {
                set_attribute!(element, Style(Style(DeclarationBlock::default())));
            }
            let mut style_attr =
                get_attribute_mut!(element, Style).expect("style should have been initialised");
            ...
            } in group
            {
                for token in tokens {
                    ...
                    match token {
                        ...
                        Token::ID { .. } => {
                            element.remove_attribute(&AttrId::Id);
                        }
                        _ => {}
                    }
                }
                    ...
                    Token::Attr { local_name, value } => match value {
                        Some((value, operator, sensitivity)) => {
                            m.get_attribute_local(local_name).is_some_and(|atom| {
                                operator
                                    .0
                                    .eval_str(&atom.value().to_string(), value, *sensitivity)
                            })
                        }
