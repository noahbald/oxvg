use oxvg_ast::{
    attribute::{data::AttrId, AttributeGroup, AttributeInfo},
...
impl<'input, 'arena> Visitor<'input, 'arena> for RemoveUnknownsAndDefaults {
    ...
    fn element(
        ...
        let inherited = ComputedStyles::default()
            .with_inherited(element, &context.stylesheet)
...
impl RemoveUnknownsAndDefaults {
    fn remove_unknown_content(&self, element: &Element<'_, '_>) {
    ...
    fn remove_unknown_and_default_attrs<'input>(
        ...
        element.attributes().retain(|attr| {
            ...
            let inheritable = name.info().contains(AttributeInfo::Inheritable);
            log::debug!("trying to remove attr: \"{name}\"");
            if *prefix == Prefix::XML || *prefix == Prefix::XLink || *prefix == Prefix::XMLNS {
                log::debug!("ignoring prefix: {prefix:?}");
                return true;
            } else if self.keep_data_attrs && local_name.starts_with("data-") {
                log::debug!("keeping data attribute");
                return true;
            } else if self.keep_aria_attrs && local_name.as_str().starts_with("aria-") {
                log::debug!("keeping aria attribute");
                return true;
            } else if self.keep_role_attr && *name == AttrId::Role {
