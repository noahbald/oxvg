...
impl RemoveDeprecatedAttrs {
    fn process_attributes<'input, 'arena>(
        &self,
        element: &Element<'input, 'arena>,
        ...
        element.attributes().retain(|attr| {
            ...
            if info.contains(AttributeInfo::DeprecatedSafe) {
                return false;
            } else if self.remove_unsafe && info.contains(AttributeInfo::DeprecatedUnsafe) {
                return false;
            }
            true
        });
