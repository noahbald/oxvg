...
/// let attr = Attr::Id("my-attr".id());
/// assert!(is_attribute!(attr, Id | Class));
/// ```
///
/// ```
/// // Matching attribute values
/// let attr = Attr::Stroke(SVGPaint::None);
/// assert!(is_attribute!(attr, Stroke(SVGPaint::None)));
/// ```
...
macro_rules! get_attribute {
    ($element:expr, $attr:ident$(,)?) => {
        $element
            ...
            .map(|attr| {
                std::cell::Ref::map(attr, |attr| match attr.unaliased() {
                    $crate::attribute::data::Attr::$attr(inner) => inner,
...
macro_rules! get_attribute_mut {
    ($element:expr, $attr:ident$(,)?) => {
        $element
            ...
            .map(|attr| {
                std::cell::RefMut::map(attr, |attr| match attr {
                    $crate::attribute::data::Attr::$attr(inner) => inner,
                    _ => unreachable!(),
                })
            })
