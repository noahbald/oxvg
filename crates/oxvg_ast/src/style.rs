...
macro_rules! get_computed_style {
    ($computed_style:expr, $id:ident$(,)?) => {
        $computed_style
            ...
            .map(|(attr, mode)| match attr {
                $crate::attribute::data::Attr::$id(inner) => (inner, mode),
                _ => unreachable!(),
            })
    };
}
...
impl<'input> ComputedStyles<'input> {
    ...
    pub fn with_all(
        ...
        styles: &Vec<RefCell<CssRuleList<'input>>>,
    ...
    pub fn get_inherited(&self, id: &str) -> Option<(Attr<'input>, Mode)> {
        self.inherited
            ...
            .and_then(|style| match style {
                ...
                Style::Static(Static::Attr(attr)) => Some((attr, Mode::Static)),
            })
    }
