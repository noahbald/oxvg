...
#[cfg(feature = "oxc_ast")]
pub fn transform<'input, 'arena, 'alloc, B, W: Write, FA, FS>(
...
where
    B: oxc_ast::builder::GetAstBuilder<'alloc>,
    FA: for<'a> FnOnce(&mut W, VariablesAST<'a>, TemplateContext) -> Result<(), TemplateError>,
    FS: FnOnce(&mut W, VariablesString, TemplateContext) -> Result<(), TemplateError>,
{
    ...
    let mut jsx =
        crate::oxc::to_jsx(code, &config, state.as_ref(), builder).map_err(Error::BuildError)?;
    let mut native_idents = HashSet::new();
    crate::oxc::preset(&mut jsx, &config, &mut native_idents, builder)?;
    let options = config.opts(state, native_idents);
    let variables =
        crate::oxc::Variables::new(jsx, &options, builder).map_err(Error::ConfigError)?;
    ...
    match template {
        ...
        None => crate::oxc::default_template(w, variables, context),
    }
...
>(
    ...
    let mut jsx = crate::swc::to_jsx(code, &config, state.as_ref()).map_err(Error::BuildError)?;
    let mut native_idents = HashSet::new();
