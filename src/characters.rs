...
pub fn char(file_reader: &mut FileReader, expected: Option<char>) -> Result<(), Box<SVGError>> {
    ...
    match expected {
        Some(x) if x != char => Err(SVGError::new_curse(
            file_reader.get_cursor(),
            SVGErrorLabel::UnexpectedChar(x, char.into()),
        ))?,
        _ => {}
    };
    Ok(())
