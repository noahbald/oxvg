...
const svgFileTree = async (subPath = ".") => {
	...
	const result = [];
	const dir = await fs.readdir(path.resolve(__dirname, originalDir, subPath));
	await Promise.all(
		dir.map(async (symbol) => {
			const newSubPath = `${subPath}/${symbol}`;
			const stat = await fs.stat(
				path.resolve(__dirname, originalDir, newSubPath),
			);
			if (stat.isDirectory()) {
				result.push(...(await svgFileTree(newSubPath)));
			} else if (symbol.endsWith(".svg")) {
				result.push({
					original: path.resolve(__dirname, originalDir, subPath, symbol),
					optimised: path.resolve(__dirname, optimisedDir, subPath, symbol),
				});
			}
		}),
	);
...
(async () => {
	...
	const brokenCount = (
		await Promise.all(svgs.map((item) => process(item)))
